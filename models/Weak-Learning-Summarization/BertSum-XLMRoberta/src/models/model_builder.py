...
class XLMRoberta(nn.Module):
    #changed lines 44, 46, 48, 50
    def __init__(self, temp_dir, load_pretrained_bert, bert_config):
        super(XLMRoberta, self).__init__()
        if(load_pretrained_bert):
            self.model = XLMRobertaModel.from_pretrained('xlm-roberta-base', cache_dir=temp_dir)
        else:
            self.model = XLMRobertaModel(bert_config)

    #changed lines 54-55
    def forward(self, x, segs, mask):
        outputs = self.model(input_ids=x, attention_mask =mask, return_dict = True)
...
class Summarizer(nn.Module):
    #changed lines 66, 77, 79
    def __init__(self, args, device, load_pretrained_bert = False, bert_config = None):
        super(Summarizer, self).__init__()
        self.args = args
        self.device = device
        self.bert = XLMRoberta(args.temp_dir, load_pretrained_bert, bert_config)
        if (args.encoder == 'classifier'):
            ...
            bert_config = XLMRobertaConfig(self.bert.model.config.vocab_size, hidden_size=args.hidden_size,
                                     num_hidden_layers=6, num_attention_heads=8, intermediate_size=args.ff_size)
            self.bert.model = XLMRobertaModel(bert_config)
            self.encoder = Classifier(self.bert.model.config.hidden_size)
